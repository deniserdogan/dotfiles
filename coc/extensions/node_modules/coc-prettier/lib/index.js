...
var require_ignore = __commonJS((exports2, module2) => {
  ...
  var Ignore = class {
    ...
    _test(originalPath, cache, checkUnignored, slices) {
      ...
      checkPath(path5, originalPath, throwError);
  ...
  var isPathValid = (path5) => checkPath(path5 && checkPath.convert(path5), path5, returnFalse);
...
function fileListener() {
  const fileWatcher = import_coc.workspace.createFileSystemWatcher(`**/{${PRETTIER_CONFIG_FILES.join(",")}}`);
...
var configCacheHandler_default = fileListener;
...
function setupErrorHandler() {
  outputChannel = import_coc2.window.createOutputChannel("prettier");
  return import_coc2.commands.registerCommand("prettier.open-output", () => {
    outputChannel.show();
  });
}

// src/ignoreFileHandler.ts
...
// src/requirePkg.ts
var import_resolve_from = __toModule(require_resolve_from());
var import_path = __toModule(require("path"));
function requireLocalPkg(fspath, pkgName, options = {silent: true, ignoreBundled: false}) {
  let modulePath = import_resolve_from.default.silent(fspath, pkgName);
  ...
  try {
    ...
    addToOutput(`Failed to load ${pkgName} from ${import_path.default.dirname(__dirname)}`, "Error");
  }
  return null;
}
...
function getConfig(uri) {
  return import_coc3.workspace.getConfiguration("prettier", uri ? uri.toString() : void 0);
}
async function getPrettierInstance() {
  ...
  const resolvedPrettier = await requireLocalPkg(import_path2.default.dirname(fileName), "prettier", {silent: true, ignoreBundled: localOnly});
  if (!resolvedPrettier) {
...
function getParsersFromLanguageId(languageId, prettierInstance, path5) {
  const supportedLanguages = getSupportLanguages(prettierInstance);
  const language = supportedLanguages.find((lang) => Array.isArray(lang.vscodeLanguageIds) && lang.vscodeLanguageIds.includes(languageId) && (lang.extensions.length > 0 || path5 != null && lang.filenames != null && lang.filenames.includes(import_path2.basename(path5))));
  if (!language) {
    return [];
  }
...
function enabledLanguages(prettierInstance) {
  ...
  const languages2 = allLanguages(prettierInstance);
  return languages2.filter((x) => disableLanguages.indexOf(x) == -1);
}
...
function getSupportLanguages(prettierInstance) {
  if (import_semver.default.gte(prettierInstance.version, "2.0.0")) {
...
function ignoreFileHandler(disposables) {
  ...
  return {
    fileIsIgnored(filePath) {
      ...
      return ignorer.ignores(path3.relative(path3.dirname(ignoreFilePath), filePath));
    }
  };
  function getIgnorerForFile(fsPath) {
    const absolutePath = getIgnorePathForFile(fsPath, getConfig(import_coc4.Uri.file(fsPath)).ignorePath);
    if (!absolutePath) {
      return {ignoreFilePath: "", ignorer: nullIgnorer};
    }
    if (!ignorers.has(absolutePath)) {
      loadIgnorer(import_coc4.Uri.file(absolutePath));
    }
    if (!import_fs.existsSync(absolutePath)) {
      const ignorePath = getConfig(import_coc4.Uri.file(fsPath)).ignorePath;
  ...
  function loadIgnorer(ignoreUri) {
    ...
    if (!ignorers.has(ignoreUri.fsPath)) {
      const fileWatcher = import_coc4.workspace.createFileSystemWatcher(ignoreUri.fsPath);
    ...
    if (import_fs.existsSync(ignoreUri.fsPath)) {
      const ignoreFileContents = import_fs.readFileSync(ignoreUri.fsPath, "utf8");
...
function getIgnorePathForFile(_filePath, ignorePath) {
  ...
  if (import_coc4.workspace.workspaceFolder) {
    const folder = import_coc4.workspace.workspaceFolder;
    return folder ? getPath(ignorePath, import_coc4.Uri.parse(folder.uri).fsPath) : null;
  }
  return null;
}
function getPath(fsPath, relativeTo) {
  return path3.isAbsolute(fsPath) ? fsPath : path3.join(relativeTo, fsPath);
}
var ignoreFileHandler_default = ignoreFileHandler;

// src/PrettierEditProvider.ts
var import_coc5 = __toModule(require("coc.nvim"));
var import_path3 = __toModule(require("path"));
var STYLE_PARSERS = ["postcss", "css", "less", "scss"];
async function resolveConfig(filePath, options) {
  try {
    const localPrettier = await requireLocalPkg(import_path3.default.dirname(filePath), "prettier", {silent: true, ignoreBundled: true});
...
function mergeConfig(hasPrettierConfig, additionalConfig, prettierConfig, vscodeConfig) {
  return hasPrettierConfig ? Object.assign({
    parser: vscodeConfig.parser,
    filepath: vscodeConfig.filepath
  }, prettierConfig, additionalConfig) : Object.assign(vscodeConfig, prettierConfig, additionalConfig);
}
async function format(text, {languageId, uri}, customOptions) {
  let u = import_coc5.Uri.parse(uri);
  const isUntitled = u.scheme == "untitled";
  const fileName = u.fsPath;
  const vscodeConfig = getConfig(u);
  const localOnly = vscodeConfig.onlyUseLocalVersion;
  ...
  if (supportedLanguages.indexOf(languageId) == -1) {
    import_coc5.window.showMessage(`${languageId} not supported by prettier`, "error");
    return;
  }
  const dynamicParsers = getParsersFromLanguageId(languageId, resolvedPrettier, isUntitled ? void 0 : fileName);
  let useBundled = false;
  let parser;
  if (!dynamicParsers.length) {
    const bundledParsers = getParsersFromLanguageId(languageId, require("prettier"), isUntitled ? void 0 : fileName);
  ...
  if (vscodeConfig.tslintIntegration && parser === "typescript") {
    return safeExecution(() => {
      const prettierTslint = requireLocalPkg(u.fsPath, "prettier-tslint").format;
  ...
  if (vscodeConfig.eslintIntegration && doesParserSupportEslint) {
    return safeExecution(() => {
      const prettierEslint = requireLocalPkg(u.fsPath, "prettier-eslint");
  ...
  if (vscodeConfig.stylelintIntegration && STYLE_PARSERS.includes(parser)) {
    const prettierStylelint = requireLocalPkg(u.fsPath, "prettier-stylelint");
    return safeExecution(prettierStylelint.format({
      text,
      filePath: fileName,
      prettierOptions
    }), text, fileName);
  ...
  if (!doesParserSupportEslint && useBundled) {
    ...
    return safeExecution(() => {
      const warningMessage = `prettier@${bundledPrettier.version} doesn't support ${languageId}. Falling back to bundled prettier@${bundledPrettier.version}.`;
      addToOutput(warningMessage, "Warning");
      return bundledPrettier.format(text, prettierOptions);
    }, text, fileName);
  }
  return safeExecution(() => resolvedPrettier.format(text, prettierOptions), text, fileName);
}
...
var PrettierEditProvider = class {
  ...
  async _provideEdits(document, options) {
    let fileName = import_coc5.Uri.parse(document.uri).fsPath;
    ...
    if (edits && edits.length && !disableSuccessMessage) {
      import_coc5.window.showMessage("Formatted by prettier");
    }
...
var PrettierEditProvider_default = PrettierEditProvider;
...
function selectors(prettierInstance) {
  let languageSelector = enabledLanguages(prettierInstance).reduce((curr, language) => {
    return curr.concat([
...
async function activate(context) {
  ...
  const {fileIsIgnored} = ignoreFileHandler_default(context.subscriptions);
  const editProvider = new PrettierEditProvider_default(fileIsIgnored);
  const statusItem = import_coc6.window.createStatusBarItem(0);
  context.subscriptions.push(statusItem);
  const config = import_coc6.workspace.getConfiguration("prettier");
  ...
  async function checkDocument() {
    await wait(30);
    let doc = import_coc6.workspace.getDocument(import_coc6.workspace.bufnr);
  ...
  function registerFormatter() {
    ...
    rangeFormatterHandler = import_coc6.languages.registerDocumentRangeFormatProvider(rangeLanguageSelector, editProvider, priority);
    formatterHandler = import_coc6.languages.registerDocumentFormatProvider(languageSelector, editProvider, priority);
  }
  registerFormatter();
  checkDocument().catch((_e) => {
  });
  import_coc6.events.on("BufEnter", async () => {
    await checkDocument();
  }, null, context.subscriptions);
  context.subscriptions.push(configCacheHandler_default());
  context.subscriptions.push(import_coc6.commands.registerCommand("prettier.formatFile", async () => {
    let document = await import_coc6.workspace.document;
    let prettierConfig = import_coc6.workspace.getConfiguration("prettier", document.uri);
    let onlyUseLocalVersion = prettierConfig.get("onlyUseLocalVersion", false);
    if (onlyUseLocalVersion && (!hasLocalPrettierInstalled(import_coc6.Uri.parse(document.uri).fsPath) || document.schema != "file")) {
